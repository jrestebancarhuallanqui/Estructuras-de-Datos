...
//    
//}
NodoArbol* eliminarRecursivo(NodoArbol*p,int dato){
    if(!p){
        return p;
    }
    NodoArbol *ptr;
    if(p->elemento>dato){
        p->izquierda=eliminarRecursivo(p->izquierda,dato);
    }
    else{
        if(p->elemento<dato){
            p->derecha=eliminarRecursivo(p->derecha,dato);
        }
        else{
            if(p->derecha or p->izquierda){
                NodoArbol*temp,*miniNodo;
                if(p->derecha==nullptr){
                    temp=p->izquierda;
                    delete p;
                    return temp;
                }
                if(p->izquierda==nullptr){
                    temp=p->derecha;
                    delete p;
                    return temp;
                }
                miniNodo=minimoArbol(p->derecha);
                p->elemento=miniNodo->elemento;
                p->derecha=eliminarRecursivo(p->derecha,miniNodo->elemento);
                return p;
            }
            else{
                delete p;
